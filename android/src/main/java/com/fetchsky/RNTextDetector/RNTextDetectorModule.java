...
public class RNTextDetectorModule extends ReactContextBaseJavaModule {
    ...
    private static double CONVERSION_FACTOR = 1000000000.0;
    ...
    @ReactMethod
    public void tesseract(final ReadableMap options, final Promise promise) {
        try {
            ...
            new AsyncTask<Void, Void, WritableArray>() {
                protected WritableArray doInBackground(Void... params) {
                    ...
                    do {
                        ...
                        tempMap.putDouble("timeConsumed", RNTextDetectorUtils.getConsumedTime(START_TIME, END_TIME));
    ...
    @ReactMethod
    public void firebase(final ReadableMap options, final Promise promise) {
        try {
            BitmapFactory.Options bitmapOptions = new BitmapFactory.Options();
            final Bitmap bitmap;
            ...
            Task<FirebaseVisionText> result =
                    detector.processImage(this.firebaseImage)
                            .addOnSuccessListener(new OnSuccessListener<FirebaseVisionText>() {
                                @Override
                                public void onSuccess(FirebaseVisionText firebaseVisionText) {
                                    END_TIME = System.nanoTime();
                                    promise.resolve(
                                            RNTextDetectorUtils.prepareFirebaseOutput(
                                                    firebaseVisionText,
                                                    RNTextDetectorUtils.getConsumedTime(START_TIME, END_TIME)));
                                }
                            })
